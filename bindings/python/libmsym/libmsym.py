...
class SymmetryOperation(Structure):
    ...
    def __str__(self):
        ...
        give_repr = '{}.{}({}{}{}{}{}, conjugacy_class: {})'.format
        return give_repr(
            __name__, self.__class__.__name__, self._names[self.type],
            order, orientation, power, axis, self.conjugacy_class)
...
class CharacterTable(Structure):
    ...
    def table(self):
        ...
        if self._table_array is None:
            self._table_array = np.ctypeslib.as_array(self._table,
...
class _ReturnCode(c_int):
    ...
    def __str__(self):
        # init is not called on the return type so we can't contruct
        # data on creation, don't decode details here, may be too late
...
def init(library_location=None):
    ...
    _lib.msymGetElements.argtypes = [_Context, POINTER(c_int),
    ...
    _lib.msymSetBasisFunctions.argtypes = [_Context, c_int,
                                           POINTER(BasisFunction)]

    _lib.msymGetBasisFunctions.restype = _ReturnCode
    _lib.msymGetBasisFunctions.argtypes = [_Context, POINTER(c_int),
    ...
    _lib.msymGetCharacterTable.argtypes = [_Context,
                                           POINTER(POINTER(CharacterTable))]
    ...
    if np is None:
        ...
        _SALCsMatrix = np.ctypeslib.ndpointer(dtype=np.float64, ndim=2,
                                              flags='C_CONTIGUOUS')
        _SALCsSpecies = np.ctypeslib.ndpointer(dtype=np.int32, ndim=1,
                                               flags='C_CONTIGUOUS')
        _NPDArray = np.ctypeslib.ndpointer(dtype=np.float64, ndim=1,
                                           flags='C_CONTIGUOUS')

    _lib.msymSymmetrySpeciesComponents.restype = _ReturnCode
    _lib.msymSymmetrySpeciesComponents.argtypes = [_Context, c_int, _NPDArray,
                                                   c_int, _NPDArray]

    _lib.msymGetSALCs.restype = _ReturnCode
    _lib.msymGetSALCs.argtypes = [_Context, c_int, _SALCsMatrix,
                                  _SALCsSpecies, POINTER(PartnerFunction)]
...
class Context(object):
    ...
    def _set_elements(self, elements):
        ...
        self._assert_success(_lib.msymSetElements(self._ctx, size,
                                                  element_array))
    ...
    def _update_elements(self):
        ...
        csize = c_int(0)
        self._assert_success(_lib.msymGetElements(self._ctx, byref(csize),
                                                  byref(celements)))
    ...
    def _update_subrepresentation_spaces(self):
        ...
        for s in srs:
            for salc in s.salcs:
                salc._update_basis_functions(basis_function_addresses,
                                             self._basis_functions)
    ...
    def set_thresholds(self, **kwargs):
        for key in kwargs.keys():
            if key not in ['zero', 'geometry', 'angle', 'equivalence',
                           'eigfact', 'permutation', 'orthogonalization']:
                raise Error('Unrecognized threshold argument')
            setattr(self._thresholds, key, kwargs[key])
        self._assert_success(_lib.msymSetThresholds(self._ctx,
                                                    pointer(self._thresholds)))
    ...
    def symmetrize_wavefunctions(self, m):
        ...
        species = np.zeros((csize), dtype=np.int32)
        self._assert_success(
            _lib.msymSymmetrizeWavefunctions(self._ctx, csize,
                                             wf, species, partner_functions))
    ...
    def generate_elements(self, elements):
        ...
        size = len(elements)
        element_array = (Element*size)(*elements)
        self._assert_success(_lib.msymGenerateElements(self._ctx, size,
                                                       element_array))
        self._update_elements()
        return self._elements
